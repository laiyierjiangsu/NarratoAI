...
def extract_timestamp_from_video_path(video_path: str) -> str:
    ...
    match_new = re.search(
        r"(?:ost[012]_)?vid_(\d{2})-(\d{2})-(\d{2})-(\d{3})@(\d{2})-(\d{2})-(\d{2})-(\d{3})\.mp4",
        filename,
    )
    if match_new:
        # 提取并格式化时间戳（包含毫秒）
        ...
        return (
            f"{start_h}:{start_m}:{start_s},{start_ms}-{end_h}:{end_m}:{end_s},{end_ms}"
        )
...
def update_script_timestamps(
    ...
    audio_result: Optional[Dict[Union[str, int], str]] = None,
    subtitle_result: Optional[Dict[Union[str, int], str]] = None,
    ...
    for item in script_list:
        ...
        if item_id and item_id in id_timestamp_mapping:
            # 根据ID找到对应的新时间戳
            item_copy["sourceTimeRange"] = id_timestamp_mapping[item_id][
                "new_timestamp"
            ]
            ...
            # 根据原始时间戳找到对应的新时间戳
            item_copy["sourceTimeRange"] = id_timestamp_mapping[orig_timestamp][
                "new_timestamp"
            ]
            current_duration = calculate_duration(item_copy["sourceTimeRange"])
            item_copy["duration"] = current_duration
        ...
        if calculate_edited_timerange and current_duration > 0:
            ...
            item_copy["editedTimeRange"] = (
                f"{start_h:02d}:{start_m:02d}:{start_s:02d}-{end_h:02d}:{end_m:02d}:{end_s:02d}"
            )
...
if __name__ == "__main__":
    ...
    video_res: Dict[Union[str, int], str] = {
    ...
    audio_res: Dict[Union[str, int], str] = {
    ...
    sub_res: Dict[Union[str, int], str] = {
    ...
    # 更新并打印结果
    updated_list_script = update_script_timestamps(
        list_script, video_res, audio_res, sub_res
    )
