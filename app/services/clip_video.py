...
def calculate_end_time(
    start_time: str, duration: float, extra_seconds: float = 1.0
) -> str:
    """
    根据开始时间和持续时间计算结束时间
...
_DEFAULT_PRE_ROLL_SECONDS = 2.0
_DEFAULT_POST_ROLL_SECONDS = 2.0
_GOAL_PRE_ROLL_SECONDS = 8.0
_GOAL_POST_ROLL_SECONDS = 12.0
_GOAL_MIN_DURATION_SECONDS = 12.0
_GOAL_MAX_DURATION_SECONDS = 30.0
_ENDING_EXTRA_POST_ROLL_SECONDS = 6.0
...
_ENDING_KEYWORDS = ("大汗淋漓", "汗", "坚持", "每个周末", "更好的自己")


def _get_segment_text(script_item: Dict) -> str:
    picture = script_item.get("picture") or ""
    narration = script_item.get("narration") or ""
    return f"{picture} {narration}"


def _seconds_to_timestamp(seconds: float, has_milliseconds: bool) -> str:
    if seconds < 0:
        seconds = 0.0
    ...
    milliseconds = int(round((secs - whole_seconds) * 1000))
...
def _compute_clip_range_seconds(
    script_item: Dict,
    tts_duration: Optional[float],
    is_last_segment: bool,
) -> Tuple[float, float, bool]:
    timestamp = script_item["timestamp"]
    start_str, end_str = parse_timestamp(timestamp)
    has_milliseconds = ("," in start_str) or ("," in end_str)

    start_seconds = utils.time_to_seconds(start_str)
    end_seconds = utils.time_to_seconds(end_str)

    if tts_duration is not None:
        end_seconds = max(end_seconds, start_seconds + float(tts_duration))

    text = _get_segment_text(script_item)
    is_goal = any(keyword in text for keyword in _GOAL_KEYWORDS)

    pre_roll = _GOAL_PRE_ROLL_SECONDS if is_goal else _DEFAULT_PRE_ROLL_SECONDS
    post_roll = _GOAL_POST_ROLL_SECONDS if is_goal else _DEFAULT_POST_ROLL_SECONDS

    if is_last_segment and any(keyword in text for keyword in _ENDING_KEYWORDS):
        post_roll += _ENDING_EXTRA_POST_ROLL_SECONDS

    clip_start = max(0.0, start_seconds - pre_roll)
    clip_end = end_seconds + post_roll

    if is_goal:
        clip_end = max(clip_end, clip_start + _GOAL_MIN_DURATION_SECONDS)
        if (clip_end - clip_start) > _GOAL_MAX_DURATION_SECONDS:
            clip_end = clip_start + _GOAL_MAX_DURATION_SECONDS

    if clip_end <= clip_start:
        clip_end = clip_start + 1.0

    return clip_start, clip_end, has_milliseconds
...
def analyze_ffmpeg_error(error_msg: str) -> str:
    ...
    if any(
        keyword in error_msg_lower
        for keyword in ["encoder", "codec", "h264", "libx264", "bitrate", "preset"]
    ):
...
def try_compatibility_fallback(
    input_path: str, output_path: str, start_time: str, end_time: str, timestamp: str
) -> bool:
    """
...
def _process_narration_only_segment(
    ...
    clip_start_seconds, clip_end_seconds, has_milliseconds = (
        _compute_clip_range_seconds(
            script_item,
            tts_item["duration"],
            is_last_segment,
        )
    )
    start_time = _seconds_to_timestamp(clip_start_seconds, has_milliseconds)
    end_time = _seconds_to_timestamp(clip_end_seconds, has_milliseconds)
...
def clip_video_unified(
    ...
    if hwaccel_type:
        ...
        logger.info(
            f"🚀 使用硬件加速: {hwaccel_type} ({hwaccel_info.get('message', '')})"
        )
    else:
        logger.info("🔧 使用软件编码")
    ...
    for i, script_item in enumerate(script_list, 1):
        ...
        logger.info(
            f"📹 [{i}/{total_clips}] 处理片段 ID:{_id}, OST:{ost}, 时间戳:{timestamp}"
        )

        try:
            is_last_segment = i == total_clips
            ...
            if (
                ...
                logger.error(
                    f"❌ [{i}/{total_clips}] 片段处理失败: OST={ost}, ID={_id}"
                )

        except Exception as e:
            failed_clips.append(f"ID:{_id}, OST:{ost}")
            logger.error(
                f"❌ [{i}/{total_clips}] 片段处理异常: OST={ost}, ID={_id}, 错误: {str(e)}"
            )

    # 最终统计
    logger.info(
        f"📊 统一视频裁剪完成: 成功 {success_count}/{total_clips}, 失败 {len(failed_clips)}"
    )
    ...
    if failed_clips:
        ...
        if len(failed_clips) == total_clips:
            ...
            logger.warning(
                f"⚠️  超过一半的片段处理失败 ({len(failed_clips)}/{total_clips})，请检查硬件加速配置"
            )
...
def clip_video(
    ...
    for i, item in enumerate(tts_result, 1):
        ...
        if duration <= 0 or duration > 300:  # 超过5分钟认为不合理
            ...
            try:
                ...
                duration = 5.0  # 默认5秒
                logger.warning(
                    f"时长修正失败，使用默认时长: {duration}秒, 错误: {str(e)}"
                )
        ...
        # 执行FFmpeg命令
        logger.info(
            f"📹 [{i}/{total_clips}] 裁剪视频片段: {timestamp} -> {ffmpeg_start_time}到{ffmpeg_end_time}"
        )
    ...
    # 最终统计
    logger.info(
        f"📊 视频裁剪完成: 成功 {success_count}/{total_clips}, 失败 {len(failed_clips)}"
    )
    ...
    if failed_clips:
        ...
        if len(failed_clips) == total_clips:
            ...
            logger.warning(
                f"⚠️  超过一半的片段裁剪失败 ({len(failed_clips)}/{total_clips})，请检查硬件加速配置"
            )
...
if __name__ == "__main__":
    video_origin_path = (
        "/Users/apple/Desktop/home/NarratoAI/resource/videos/qyn2-2无片头片尾.mp4"
    )
    ...
    tts_result = [
        ...
        },
        {
            ...
            "duration": 7.675,
            "text": "但想见庆帝，哪有那么容易？范闲艺高人胆大，竟然选择了最激进的方式——闯宫！",
        },
    ]
    subclip_path_videos = {
